...
void camel_sender_on_packet_ack(camel_sender_t* s, const uint8_t* payload, int payload_size)
{
	...
	for (uint32_t i = 0; i < msg.sample_count; i++) {
		...
		if (camel_packet_history_get(&s->packet_history, msg.samples[i].transport_seq, &e) != 0)
			continue;
		...
		if (s->in_fallback) {
			uint32_t target = camel_gcc_fallback_on_packet(&s->gcc_fallback, e.send_ts_us, msg.samples[i].recv_ts_us, e.payload_size);
