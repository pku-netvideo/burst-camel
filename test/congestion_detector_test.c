...
static int test_basic_congestion_detection(void)
{
    ...
    result = camel_congestion_detector_add_sample(&det, 10000, 50000, ts_ms++);
    FCC_EXPECT_EQ("No congestion initially", result.congested, 0);
    FCC_EXPECT_DOUBLE_EQ("Gamma = 1.0 initially", result.gamma, 1.0, 0.001);
    
    for (int i = 0; i < 7; i++) {
        result = camel_congestion_detector_add_sample(&det, 10000 + i * 5000, 50000 + i * 30000, ts_ms++);
    }
...
static int test_gamma_window_scaling(void)
{
    ...
    for (int i = 0; i < 10; i++) {
        result = camel_congestion_detector_add_sample(&det, 10000 + i * 2000, 50000 + i * 20000, ts_ms++);
    }
